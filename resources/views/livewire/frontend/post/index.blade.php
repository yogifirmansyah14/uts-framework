<div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    @foreach ($authors as $author)
                        <label class="d-block">
                            <input type="radio" wire:model="authorsInput" value="{{ $author->id }}">
                            {{ $author->name }}
                        </label>
                    @endforeach
                </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    @foreach ($categories as $category)
                        <label class="d-block">
                            <input type="radio" wire:model="categoriesInput" value="{{ $category->id }}">
                            {{ $category->name }}
                        </label>
                    @endforeach
                </div>
    ...
    <div class="row">
        {{-- <div class="col-md-12 text-center">
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <small>Author by <a href="{{ url('posts/author/' . $posts[0]->user->name) }}"
                            class="text-decoration-none">{{ $posts[0]->user->name }}</a> in <a
                            href="{{ url('posts/category/' . $posts[0]->category->slug) }}"
                            class="text-decoration-none">{{ $posts[0]->category->name }}</a> Category</small>
        ...
        @empty
            <div class="col-md-12">
                <div class="p-2">
                    <img src="{{ asset('assets/images/not-found.svg') }}" height="200rem" class="card-img-top"
                        alt="...">
                    <h4 class="mt-3 text-center">No Post Available</h4>
                </div>
            </div>
        @endforelse
    </div>
